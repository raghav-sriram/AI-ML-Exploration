...
def display(classification,image): # DEMO MNIST BLACK
...
def accuracy(w, b, data): # DEMO MNIST BLACK
    misclassified = 0
    n = len(data)
    for x, y in data:
        yy = np.argmax(y)
...
w,b = algorithm(w,b,mist,0.3,epochs=1,saveMNIST=False,distort = True) # DEMO MNIST BLACK
