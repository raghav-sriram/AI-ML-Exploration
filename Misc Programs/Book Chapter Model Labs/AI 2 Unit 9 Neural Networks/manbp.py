...
def augment_image(image1d):
    outcomes = ["shiftup", "shiftdown", "shiftleft", "shiftright", "rotateright", "rotateleft", "normal"]

    outcome = np.random.choice(outcomes)
    image2d = image1d.reshape(28, 28)

    # if outcome == "shiftup":
    #     image2d = np.roll(image2d, 1, axis=0)
    # elif outcome == "shiftdown":
    #     image2d = np.roll(image2d, -1, axis=0)
    # elif outcome == "shiftleft":
    #     image2d = np.roll(image2d, -1, axis=1)
    # elif outcome == "shiftright":
    #     image2d = np.roll(image2d, 1, axis=1)
    # elif outcome == "rotateright":
    #     image2d = ndimage.rotate(image2d, 15, reshape=False) # reshape to stay 28x28
    # elif outcome == "rotateleft":
    #     image2d = ndimage.rotate(image2d, -15, reshape=False)
    # elif outcome == "normal":
    #     pass
    # else:
    #     print(f"something's very wrong, outcome: {outcome}")
    #     sys.exit(1)
    
    return image2d.reshape(-1, 1) # return as 1d array 


def train_network(A_vec, A_prime_vec, inputs, outputs, w, b, epochs, lr, lr_scaler=None, test_accuracy=False, save_wb=False, augment=False): # save_wb = True
    # (x,y) --> input, output
    # w     --> weights
    # b     --> biases
    # lr    --> learning rate
    ...
    for epoch in range(epochs):
        ...
        if lr_scaler:
            ...
            lr_scaled = lr
        for _i, (x, y) in enumerate(dataset):
            if augment: # augment image
                x = augment_image(x)
            if _i % 250 == 0:
                print(f"[DEBUG] Epoch {epoch}: {_i}/{dataset_size}", end="\r")
        ...
        if not test_accuracy:
            ...
            if dataset_size < 15000:
                ...
                print()
                print(f"[DEBUG] Error {epoch}: {accuracy_list(w, b, inputs, outputs, sigmoid)}")
        if save_wb: # saves with pickle mode, use np version 1.23.5 at latest
            np.save(f"MNIST/weights_{epoch}", w)
            np.save(f"MNIST/biases_{epoch}", b)
...
def train_mnist():
    ...
    weights, biases = train_network(
        sigmoid, sigmoid_prime, train_x, train_y, weights, biases, 200, 0.15, lr_scaler, True, True, True
    )
...
def load_test_mnist():
    # weights 37 are aug, weights 9 are normal
    weights = np.load("MNIST/weights_37_good.npy", allow_pickle=True)
    biases = np.load("MNIST/biases_37_good.npy", allow_pickle=True)
