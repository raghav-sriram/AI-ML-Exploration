# Name: Raghav Sriram        Date: Friday 10/7/2022
...
class HeapPriorityQueue():
   ...
   def heapUp(self, k):
      ...
      if 1 < k:
         node = k//2
         if self.queue[node] > self.queue[k]:
            self.swap(k,node)
            self.heapUp(node)
      #pass 
   ...
   def heapDown(self, k, size):
      l, r, s = 2*k,2*k+1,k
      if l <= size:
         if self.queue[l] < self.queue[s]: s = l
      if r <= size:
         if self.queue[r] < self.queue[s]: s = r
      if not s == k:
         self.swap(k,s)
         self.heapDown(s,size)
   
   # make the queue as a min-heap            
   def reheap(self):
      for j in range(len(self.queue)//2, 0, -1): self.heapUp(j) #,len(self.queue)-1
      #pass
   ...
   def pop(self):
      # Your code goes here
      #root = 1
      #pop = self.queue.pop(root)
      #self.reheap()
      #return pop
      return self.remove(1)    # change this
      
   # remove a value at the given index (assume index 0 is the root)
   # return the removed value   
   def remove(self, index):
      # Your code goes here
      rem = self.queue.pop(index)
      self.heapDown(1,abs(1-len(self.queue)))
      #self.reheap()
      return rem
...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   # Your code goes here
   fax = open(nodes)
   for node in fax.readlines():
      n, y, x= node.rstrip().split(" ")
   ...
   for i in fax:
      ...
      price = calc_edge_cost(*nodeLoc[x], *nodeLoc[y])
      edgeCost[(x,y)] = price
      edgeCost[(y,x)] = price
...
def display_path(path, graph):
   ...
   # Your code goes here
   print ([graph[1][pat] for pat in path if pat in graph[1]])
   pass
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   ...
   # Your code goes here
   for pat in range(len(path)-1):
      drawLine(canvas, *graph[5][path[pat]], *graph[5][path[pat+1]], col)
      ROOT.update()
   pass
   ...
   while frontier[0] and frontier [1]:
      ...
      while temp:
         ...
         if s in frontier[1-k]:
            p1, c1 = generate_path(s, explored[0], graph)
            p2, c2 = generate_path(s, explored[1], graph)
            p, c = p1[:-1]+p2[::-1],c1+c2
            draw_final_path(ROOT, canvas, p, graph)
            print("Length of the path:", len(path))
            return p,c
         for i in graph[3][s]:
            if i not in explored[k]:
               frontier[k].append(i)
               explored[k][i]=s
               drawLine(canvas, *graph[5][s], *graph[5][i],col)
         counter +=1
      if counter %1000 == 0:
         ROOT.update()
