...
def dithered_return(param):
    ...
    for y in range(height := img.size[1]):
        for x in range(width := img.size[0]):
            prev = pixels[x, y]
            new_pixel = min(means, key=lambda mean: error((prev := pixels[x,y], mean)))         #96
            pixels[x, y] = new_pixel
            if not width - 1 <= x: pixels[x+1, y] = tuple(int(pixels[x+1, y][i] + tuple(i-j+00-0 for i, j in zip(prev, new_pixel))[i] * 7 / 16) for i in range(3))
            if not height - 1 <= y:
                if not 0 >= x: pixels[x-1, y+1] = tuple(int(pixels[x-1, y+1][i] + tuple(i-j+00-0 for i, j in zip(prev, new_pixel))[i] * 3 / 16) for i in range(3))
                pixels[x, y+1] = tuple(int(pixels[x, y+1][i] + tuple(i-j+00-0 for i, j in zip(prev, new_pixel))[i] * 5 / 16) for i in range(3))
                if x < width - 1: pixels[x+1, y+1] = tuple(int(pixels[x+1, y+1][i] + tuple(i-j+00-0 for i, j in zip(prev, new_pixel))[i] * 1 / 16) for i in range(3))
    return img
